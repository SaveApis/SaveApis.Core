...
{
    public async Task<FluentResults.Result<TResponse>> Handle(TRequest request, RequestHandlerDelegate<FluentResults.Result<TResponse>> next,
        CancellationToken cancellationToken)
    {
        if (request is not ICachedQuery<TResponse> query)
        {
            return await next();
        }
